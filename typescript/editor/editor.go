...
//  +------------------------------------------------------------+
//
// 	import "github.com/radicalmind/xeon/editor"
//  [...]
//
//  app := xeon.New()
...
import (
	...
	"github.com/radicalmind/xeon/typescript/npm"
)

type (
	// Editor is the alm-tools adaptor.
	//
	// It holds a logger from the xeon' station
...
// GetDescription EditorPlugin is a bridge between Xeon and the alm-tools, the browser-based IDE for client-side sources.
func (e *Editor) GetDescription() string {
	return "A bridge between Xeon and the alm-tools, the browser-based IDE."
}
...
func (e *Editor) build(s *xeon.Application) {
	e.log = s.Log
...
// close kills the editor's server when Xeon is closed
func (e *Editor) close(s *xeon.Application) {
	if e.process != nil {
		err := e.process.Kill()
		if err != nil {
...
func (e *Editor) start() {
	...
	// to the io.Writer that xeon' user is defined from configuration
...
// Attach adapts the editor to one or more Xeon instance(s).
func (e *Editor) Attach(app *xeon.Application) {
