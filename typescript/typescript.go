...
type (
	// Typescript contains the unique xeon' typescript loader, holds all necessary fields & methods.
	Typescript struct {
		Config *Config
...
// Attach attaches the typescript to one or more Xeon instance(s).
func (t *Typescript) Attach(app *xeon.Application) {
	t.log = app.Log
	t.start()
}
