// Copyright 2017 ΓΜ & Go Authors. All rights reserved.
...
//
// You can find the Route by xeon.Default.Routes().Lookup("theRouteName")
// you can set a route name as: myRoute := xeon.Default.Get("/mypath", handler)("theRouteName")
// that will set a name to the route and returns its xeon.Route instance for further usage.
//
...
//
// Developers can wrap this handler using the `xeon.StripPrefix`
...
// app := xeon.New()
// ...
// fileserver := xeon.StaticHandler("./static_files", false, false)
// h := xeon.StripPrefix("/static", fileserver)
...
// use that or the xeon.StaticHandler/StaticWeb methods
...
// it doesn't cares about the rest of your xeon configuration.
//
// Use the xeon.StaticHandler/StaticWeb in order to serve static files on more automatic way
...
//  | custom xeon http errors and fallback to non-compressed data|
//  | when not supported.                                        |
//  |                                                            |
//  +------------------------------------------------------------+
