...
import (
	...
	"github.com/radicalmind/xeon/core/router/macro"
)
...
// so `xeon.StripPath` is useless and should not being used here.
//
// Usage:
// app := xeon.New()
// ...
...
//
// Returns the read-only route information of the GET route.
...
// Returns the read-only route information of the GET route.
//
// Example: https://github.com/radicalmind/xeon/tree/master/_examples/intermediate/serve-embedded-files
func (rb *APIBuilder) StaticEmbedded(requestPath string, vdir string, assetFn func(name string) ([]byte, error), namesFn func() []string) (*Route, error) {
	...
	h := func(ctx context.Context) {
		...
		for _, path := range names {
			...
			// as requested here: https://github.com/radicalmind/xeon/issues/633#issuecomment-281691851
			if path == "/index.html" {
