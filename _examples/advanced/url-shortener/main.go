// Package main shows how you can create a simple URL SHortener using only Xeon and BoltDB.
//
...
func main() {
	...
	execShortURL := func(ctx context.Context, key string) {
		...
		value := db.Get(key)
		if value == "" {
			ctx.StatusCode(xeon.StatusNotFound)
			ctx.Writef("Short URL for key: '%s' not found", key)
			return
		}

		ctx.Redirect(value, xeon.StatusTemporaryRedirect)
	}
