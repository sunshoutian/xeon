...
func main() {
	...
	app.Get("/change", func(ctx context.Context) {
		...
		if none.IsOnline() {
			none.Method = xeon.MethodNone
		} else {
			none.Method = xeon.MethodGet
		}
	...
	app.Get("/execute", func(ctx context.Context) {
		// same as navigating to "http://localhost:8080/invisible/radicalmind" when /change has being invoked and route state changed
		// from "offline" to "online"
		ctx.Values().Set("from", "/execute") // values and session can be shared when calling Exec from a "foreign" context.
		ctx.Exec("GET", "/invisible/radicalmind")
	})
