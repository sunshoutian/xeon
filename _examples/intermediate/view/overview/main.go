...
func main() {
	...
	app.Get("/markdown", func(ctx context.Context) {
		ctx.Markdown([]byte("# Hello Dynamic Markdown -- Xeon"))
	})

	//

	// The part we care in this section is the following:
	app.AttachView(view.HTML("./templates", ".html"))
	app.Get("/template", func(ctx context.Context) {

		ctx.ViewData("Name", "Xeon") // the .Name inside the ./templates/hi.html
