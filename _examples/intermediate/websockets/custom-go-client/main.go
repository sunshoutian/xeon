...
import (
	...
	"github.com/radicalmind/xeon/websocket"
)
...
func SendtBytes(serverID, to, method string, message []byte) error {
	// look https://github.com/radicalmind/xeon/tree/master/adaptors/websocket/message.go , client.go and client.js
	// to understand the buffer line:
	buffer := []byte(fmt.Sprintf("xeon-websocket-message:%v;0;%v;%v;", method, serverID, to))
