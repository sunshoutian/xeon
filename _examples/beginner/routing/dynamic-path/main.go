...
func main() {
	...
	// Xeon, like net/http std package registers route's handlers
	// by a Handler, the Xeon' type of handler is just a func(ctx context.Context)
	// where context comes from github.com/radicalmind/xeon/context.
	// Until go 1.9 you will have to import that package too, after go 1.9 this will be not be necessary.
	//
	// Xeon has the easiest and the most powerful routing process you have ever meet.
	// If you're used to use the "httprouter"
	// then you don't have to change a thing of a route's path.
	//
	// At the same time,
	// Xeon has its own interpeter(yes like a programming language)
	...
	//
	// Note: the Xeon' router follows the "httprouter"'s rules for routes confliction.
	//
	...
	//
	//
	// Besides the fact that Xeon provides the basic types and some default "macro funcs"
	...
	//
	// {param:string equal(xeon)} , "xeon" will be the argument here:
	...
	// http://localhost:8080/lowercase/radicalmind
