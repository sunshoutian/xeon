...
func main() {
	...
	// fires when route not found or manually by ctx.StatusCode(xeon.StatusNotFound).
	app.OnErrorCode(xeon.StatusNotFound, notFoundHandler)
	...
	app.Handle("GET", "/", func(ctx context.Context) {
		// navigate to the middle of $GOPATH/src/github.com/radicalmind/xeon/context/context.go
		// to overview all context's method (there a lot of them, read that and you will learn how xeon works too)
	...
	adminRoutes.Get("/", func(ctx context.Context) {
		// [...]
		ctx.StatusCode(xeon.StatusOK) // default is 200 == xeon.StatusOK
...
func donateHandler(ctx context.Context) {
	...
	ctx.Values().Set("donate_url", "https://github.com/radicalmind/xeon#buy-me-a-cup-of-coffee")
...
func donateFinishHandler(ctx context.Context) {
	...
	// but Xeon provides an easy to do that, if donate_url is not defined, then it returns an empty string instead.
