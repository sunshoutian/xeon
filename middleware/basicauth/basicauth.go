...
// use: xeon.Use(New(...)), xeon.Get(...,New(...),...)
func New(c Config) context.Handler {
	config := DefaultConfigurationReadOnly()
	if c.ContextKey != "" {
...
// use: xeon.Use(Default(...)), xeon.Get(...,Default(...),...)
...
func (b *basicAuthMiddleware) askForCredentials(ctx context.Context) {
	...
	ctx.StatusCode(xeon.StatusUnauthorized)
}
