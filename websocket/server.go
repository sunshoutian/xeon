...
type Server interface {
	// Attach adapts the websocket server to an Xeon instance.
	// see websocket.go
	Attach(app *xeon.Application)

	// Handler returns the xeon.Handler
...
func (s *server) Handler() context.Handler {
	...
	return func(ctx context.Context) {
		...
		if err != nil {
			...
			ctx.StatusCode(xeon.StatusServiceUnavailable)
			return
		}
