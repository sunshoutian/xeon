...
// The Context is the most important thing on the Xeon' http flow.
//
...
type Context interface {
	...
	//
	// To follow the Xeon' flow, developer should:
	...
	//
	// Example: https://github.com/radicalmind/xeon/tree/master/_examples/intermediate/i18n
	...
	// to be executed. Next handlers are being executed on Xeon because you can alt the
	...
	//
	//     * if response body is too big (more than xeon.LimitRequestBodySize(if setted)).
	...
	//
	// Note that the 'layoutTmplFile' argument can be setted to xeon.NoLayout || view.NoLayout
	// to disable the layout for a specific view render action,
	// it disables the engine's configuration's layout property.
	//
	// Look .ViewData and .View too.
	//
	// Example: https://github.com/radicalmind/xeon/tree/master/_examples/intermediate/view/context-view-data/
	...
	//
	// Examples: https://github.com/radicalmind/xeon/tree/master/_examples/intermediate/view/
	...
	// (=I haver never seen any other examples or code in Golang on this subject, so far, as with the most of xeon features...)
	// it's not covers all paths,
	// such as databases, this should be managed by the libraries you use to make your database connection,
	// this transaction scope is only for context's response.
	// Transactions have their own middleware ecosystem also, look xeon.go:UseTransaction.
	//
	// See https://github.com/radicalmind/xeon/tree/master/_examples/advanced/transactions for more
	...
	//
	// Offline means that the route is registered to the xeon and have all features that a normal route has
	...
	//
	// Example: https://github.com/radicalmind/xeon/tree/master/_examples/intermediate/route-state
	//
	...
	// Application returns the Xeon framework instance which belongs to this context.
...
func (ctx *context) Redirect(urlToRedirect string, statusHeader ...int) {
	...
	// the
	// Fixes: http: //support.xeon-go.com/d/21-wrong-warning-message-while-redirecting
	shouldCheckForCycle := urlToRedirect == ctx.Path() && ctx.Method() == http.MethodGet
	// from POST to GET on the same path will give a warning message but developers don't use the xeon.DevLogger
